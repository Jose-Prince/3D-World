...
impl Obj {
    ...
    pub fn get_vertex_array(&self) -> Vec<Vertex> {
        ...
        // Iteramos sobre los índices para construir el array de vértices
        for &index in &self.indices {
            let position = self.vertices[index as usize];
            let normal = if !self.normals.is_empty() {
                self.normals[index as usize]
            } else {
                Vec3::new(0.0, 0.0, 0.0) // Normal por defecto si no hay normales en el archivo
            };
            let tex_coords = if !self.texcoords.is_empty() {
                self.texcoords[index as usize]
            } else {
                Vec2::new(0.0, 0.0) // Coordenadas UV por defecto si no hay texcoords en el archivo
            };
    
            // Creamos un vértice con la posición, normal y texcoords correspondientes
